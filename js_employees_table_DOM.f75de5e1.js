...
function sorting(idx) {
    if (lastSortedColumn === idx) lastDirectionAsc = !lastDirectionAsc;
    else {
    ...
    const sorted = [
        ...
        else return (aVal - bVal) * direction;
    });
    renderTable(sorted);
}
...
function renderTable(rows) {
    ...
    rows.forEach((person)=>{
        ...
        [
            ...
            if (key === 'salary') td.textContent = priceFormat(person[key]);
            else td.textContent = person[key];
            tr.appendChild(td);
        });
...
function createForm() {
    ...
    [
        ...
        if (el === 'name' || el === 'position') formEl.setAttribute('type', 'text');
        if (el === 'age' || el === 'salary') formEl.setAttribute('type', 'number');
        if (el === 'office') [
            'Tokyo',
            'Singapore',
            'London',
...
function attachFormHandler(form) {
    ...
    form.addEventListener('submit', (e)=>{
        ...
        if (empty.length > 0) {
            pushNotification(500, 10, 'Error - required fields', 'Please fill all fields before submitting.', 'error');
            return;
        }
        const nameMin = 4;
        const ageMin = 18;
        ...
        if (nameInput.value.trim().length < nameMin) {
            pushNotification(500, 10, 'Error - not correct Name field', 'Name should be more than ' + nameMin + ' symbols.\n ', 'error');
            return;
        }
        if (position.value.trim().length < 1 && salary.value.trim().length < 1) {
            pushNotification(500, 10, 'Error', 'Fields should be not empty.\n ', 'error');
            return;
        }
        if (Number(age.value.trim()) < ageMin || Number(age.value.trim()) > ageMax) {
            pushNotification(500, 10, 'Error - not correct Age field', 'Age value is more ' + ageMin + ' or less than ' + ageMax + '.\n ', 'error');
            return;
        }
        pushNotification(500, 10, 'Success', 'New peron was added to table.', 'success');
        const obj = {
